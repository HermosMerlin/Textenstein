...
RayHit ray_check_dda(const Map* map, double x, double y, double angle, double max_distance) {
    ...
    double delta_dis_x = (fabs(dir_x) < eps) ? inf : fabs(1.0 / dir_x);
    double delta_dis_y = (fabs(dir_y) < eps) ? inf : fabs(1.0 / dir_y);

    int step_x = (dir_x >= 0) ? 1 : -1;
    int step_y = (dir_y >= 0) ? 1 : -1;

    double side_dis_x = (dir_x >= 0)
                        ? (ray.map_x + 1.0 - x) * delta_dis_x
                        : (x - ray.map_x) * delta_dis_x;
    double side_dis_y = (dir_y >= 0)
    ...
    while (dis < max_distance) {
        ...
        if (map_is_wall(map, ray.map_x, ray.map_y)) {
            ray.hit = 1;
            ray.hit_x = x + dir_x * dis;
            ray.hit_y = y + dir_y * dis;
            ray.distance = dis;
            return ray;
        }
    }
